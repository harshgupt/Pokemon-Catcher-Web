...
const REGIONS_WITH_BGS = new Set([
	"Kanto",
	"Johto",
	"Hoenn",
	"Sinnoh",
...
const BG_POOL = Object.entries(locationsData)
	...
	.flatMap(([r, locs]) =>
		Object.keys(locs).map((l) => ({ region: r, location: l })),
	);
const FORMS = [
		...
		for (const nf of p.nextForms) {
			...
			if (!nextUnlocked) {
				...
				result.push({
					...
					fromSrc: fromPoke
						? assetUrl(
								`/sprites/pokemon/mid/${fromPoke.spriteName ?? fromPoke.name}.png`,
							)
						: "",
					nextSrc: nextPoke
						? assetUrl(
								`/sprites/pokemon/mid/${nextPoke.spriteName ?? nextPoke.name}.png`,
							)
						: "",
...
export default function CatchTab({
	...
	const regionLocations = region
		? Object.keys(locationsData[region] ?? {})
		: [];
	...
	useEffect(() => {
		...
		const onVisibility = () => {
			if (document.hidden) saveGame(gsRef.current);
		};
	...
	const availableFilters = useMemo(() => {
		const activeIds = new Set(
			pokemonData
				.filter((p) => (gameState.pokemon[p.id]?.numberToSpawn ?? 0) > 0)
				.map((p) => p.id),
		);
		const types = new Set();
	...
	}, [type1, type2, region, location, form, cls, itemsOnly]);
	...
	function doGenerateGrid(gs) {
		...
		if (getGlobalTokens(gs) === 0) {
			setPhase("gameOver");
			setSlots([]);
			setHintSlots([]);
			return;
		}
		if (getAvailableTokens(gs, f) === 0) {
			setPhase("categoryEmpty");
			setSlots([]);
			setHintSlots([]);
			return;
		}
	...
	const clue =
		hoveredIdx !== -1 && slots[hoveredIdx] ? slots[hoveredIdx].clue : null;
	...
	const bgAnimation = panAxis
		? `bg-pan-${panAxis} ${panDuration.toFixed(1)}s linear infinite`
		: undefined;
			...
			{phase !== "gameOver" && (
				<div style={styles.filterBar}>
					<select
						...
						disabled={filtersDisabled || availableFilters.types.size === 0}>
						<option value="">Type 1</option>
						{TYPES.filter((t) => availableFilters.types.has(t)).map((t) => (
					...
					<select
						...
						disabled={filtersDisabled || availableFilters.regions.size === 0}>
						<option value="">Region</option>
						{REGIONS.filter((r) => availableFilters.regions.has(r)).map((r) => (
					...
					{region && (
						<select
							...
							disabled={
								filtersDisabled ||
								(availableFilters.locs[region]?.size ?? 0) === 0
							}>
							<option value="">Location</option>
							{regionLocations
								.filter((l) => availableFilters.locs[region]?.has(l))
								.map((l) => (
					...
					<select
						style={{ ...styles.filterSelect, ...styles.filterSelectWide }}
						value={form}
						onChange={(e) => setForm(e.target.value)}
						disabled={filtersDisabled || availableFilters.forms.size === 0}>
						<option value="">Form</option>
						{FORMS.filter((f) => availableFilters.forms.has(f.value)).map(
							(f) => (
					...
					<select
						...
						disabled={filtersDisabled || availableFilters.classes.size === 0}>
						<option value="">Class</option>
						{CLASSES.filter((c) => availableFilters.classes.has(c.value)).map(
							(c) => (
					...
					<button
						style={{
							...styles.itemsBtn,
							...(itemsOnly ? styles.itemsBtnActive : {}),
						}}
					...
					<button
						...
						onClick={() => doGenerateGrid(gsRef.current)}
						title="Refresh grid">
						↻
					</button>
				</div>
				...
				{phase === "gameOver" && (
					<div style={styles.endPanel}>
						...
						<p style={styles.endSub}>
							You've caught everything. Start a new game?
						</p>
					...
					<>
							...
							<div style={styles.grid}>
								{slots.map((_slot, idx) => (
									<BallSlot
										...
										onHoverEnter={() => {
											if (animIdx === -1) setHoveredIdx(idx);
										}}
						...
						<div style={styles.clueBar}>
							{clue ? (
								...
								<span style={styles.cluePlaceholder}>
									Hover a Pokéball for a hint…
								</span>
							)}
						</div>
					</>
...
function BallSlot({
	...
	return (
		<div
			...
			onClick={onClick}>
			<img
				src={assetUrl("/sprites/pokeball.png")}
				alt="Pokéball"
				style={imgStyle}
				draggable={false}
			/>
	...
	return (
		<div style={{ ...styles.overlay, perspective: "1200px" }} onClick={onClose}>
			<div
				style={{
					...styles.popup,
					...(isFirstCatch ? styles.popupFirstCatch : {}),
					animation: "popup-flip-in 0.35s ease-out",
				}}
					...
					style={
						isFirstCatch
							? {
									...
									animation: "first-catch-glow 2.5s ease-in-out 1 forwards",
								}
							: d.isItem
								? styles.popupImageItem
								: styles.popupImage
					}
					onError={(e) => {
						...
						{evolutions.map((ev, i) => {
							...
							const itemSrc = item
								? item.tmType
									...
									: assetUrl(`/sprites/items/${item.name}.png`)
								: null;
							const reqSrc = reqChar
								? assetUrl(
										`/sprites/pokemon/mid/${reqChar.spriteName ?? reqChar.name}.png`,
									)
								: null;
							const itemHeld = item
								? (gameState.items[item.id]?.numberCollected ?? 0) > 0
								: true;
							const charMet = reqChar
								? (gameState.pokemon[reqChar.id]?.isUnlocked ?? false)
								: true;
							return (
								<div key={i} style={styles.evoChip}>
									...
									{reqSrc && (
										<img
											...
											style={{
												...styles.evoSprite,
												opacity: charMet ? 1 : 0.35,
											}}
										/>
									)}
									{ev.canEvolve && (
										<button
											className="btn-evolve-golden"
											onClick={(e) => {
												e.stopPropagation();
												onEvolve(ev);
											}}>
